...
**调研日期**: 2026-04-05（2026-04-11 更新：补充使用路径、agent 触发方式、Sisyphus vs ulw 差异）
**调研方法**: 3 个并行 sub-agent 交叉验证 + 初步扫描 + librarian 二次验证
...
| **Sisyphus-Junior** | 并行实现 Worker | Sisyphus 的子实例，执行主 Sisyphus 拆分出的具体子任务，可多个并行跑 | ulw 模式下由主 Sisyphus 通过 `task()` 自动派发 |
...
来源：[orchestration.md](https://github.com/code-yeongyu/oh-my-openagent/blob/dev/docs/guide/orchestration.md)
...
这是简单任务/快速修复？
  ...
  └─ 否 → 解释完整上下文很麻烦？
              └─ 是 → 输入 ulw（Sisyphus + ultrawork / ulw-loop）
              └─ 否 → 需要精确可验证的执行？
                         └─ 是 → @plan → /start-work（Prometheus + Atlas）
                         └─ 否 → 还是用 ulw
```

官方将其归纳为 **3 条决策路径 + Hephaestus 作为 ulw 的 power-user 变体**，加上 `/ralph-loop` 作为轻量持续执行路径：

| 官方名称 | 路径 | 触发方式 |
|---------|------|---------|
| "Just prompt" | 普通 Sisyphus | 直接输入，无需任何特殊操作 |
| "Complex + Lazy" | Sisyphus + ulw（= `/ulw-loop`） | 输入 `ulw` 或 `ultrawork` |
| "Complex + Iterative" | `/ralph-loop` | `/ralph-loop "任务描述"` |
| "Complex + Precise" | Prometheus + Atlas | Tab 切换到 Prometheus → 规划 → `/start-work` |
| Power-user variant | Hephaestus | Tab 切换到 Hephaestus（**GPT-only，Claude 环境无效**）|

### 五种路径完整对比

| | 普通 Sisyphus | Sisyphus + ulw | `/ralph-loop` | Prometheus + Atlas | Hephaestus |
|---|---|---|---|---|---|
| **适合任务** | 简单/单文件/快速修复 | 复杂但懒得解释 | 多步骤、需多轮跑完 | 复杂且需要精确可验证 | 深度架构推理/复杂调试 |
| **触发方式** | 直接输入 | 输入 `ulw` | `/ralph-loop "任务"` | Tab→Prometheus→`/start-work` | Tab 切换到 Hephaestus |
| **自动规划** | ❌ | ✅ 自动生成 | ❌ | ✅ Prometheus 采访式规划 | ✅ 自包含内部规划 |
| **子 agent 派发** | 按需 | ✅ 并行 Sisyphus-Junior | 按需 | ✅ Atlas 分配 category tasks | ❌ 禁止派实现类子 agent |
| **持续循环** | ❌ | ✅ 跑完为止 | ✅ 最多 100 轮（可配置）| ✅ Atlas 独立验证 | ✅ 内部 verify 循环 |
| **完成验证** | 无 | Oracle 验证 | agent 自判断 | Atlas 独立验证 | 内部 verify |
| **人工介入** | 正常对话 | 极少（walk away）| 极少 | 规划阶段需参与 | 极少 |
| **模型** | Claude Opus | Claude Opus | Claude Opus | 规划 Claude，执行按 category | **强制 GPT-5.4**（非 GPT 被拦截）|
| **生成计划文件** | ❌ | 可选（有则复用）| ❌ | ✅ `.sisyphus/plans/*.md` | ❌ |

### 普通 Sisyphus vs Sisyphus + ulw 的精确差异
...
| 能力 | 普通 Sisyphus | Sisyphus + ulw |
|------|-------------|----------------|
| 自动规划 | ❌ 响应你的 prompt | ✅ 自动生成执行计划 |
| 深度调研 | 按需派 explore/librarian | ✅ 主动激进派发 |
| 并行 agent | 按需 | ✅ 激进并行 Sisyphus-Junior |
| 自我纠错 | ❌ | ✅ Oracle 验证循环 |
| 持续执行 | 单轮或短循环 | ✅ 跑完为止，不停止 |
| `ulw` 对 Hephaestus 的效果 | — | 几乎无额外效果（Hephaestus 默认就是 ultrawork 强度）|

### Agent 角色分类：手动 vs 自动

| Agent | 类型 | 触发方式 |
|-------|------|---------|
| **Sisyphus** | 主 orchestrator | 自动（每次对话入口）|
| **Explore** | 代码库搜索 | 自动（Sisyphus 判断需要时派发）|
| **Librarian** | 外部文档/库搜索 | 自动（遇到不熟悉的库时派发）|
| **Oracle** | 架构顾问 | 自动（2次修复失败、复杂架构问题时）|
| **Metis** | 需求 gap 分析 | 自动（需求模糊时咨询）|
| **Momus** | 计划审核 | 自动（计划写完后审核）|
| **Prometheus** | 采访式规划 | **手动**（Tab 键切换）|
| **Atlas** | 计划执行调度 | **手动**（`/start-work` 触发）|
| **Hephaestus** | 深度自主执行 | **手动**（Tab 切换，GPT-only）|
| **Sisyphus-Junior** | 并行实现 worker | 自动（ulw 模式下由 Sisyphus 派发）|
...
### 2. Ralph Loop（轻量持续循环执行）

适合任务明确但步骤多、可能需要多轮才能跑完的场景。与 `ulw` 的核心区别：`ulw` 是单次 prompt 增强，Ralph Loop 是系统级循环机制——每轮结束后系统自动注入 continuation prompt 推动下一轮，直到 agent 输出完成信号。
...
| 参数 | 默认值 | 说明 |
|---|---|---|
| `--completion-promise` | `DONE` | 自定义完成信号文本 |
| `--max-iterations` | 100 | 最大迭代次数上限 |
| `--strategy=reset` | — | 每轮开新 session |
| `--strategy=continue` | — | 同一 session 内继续 |

**退出条件**：agent 输出 `<promise>DONE</promise>` → 循环结束；或达到迭代上限；或用户运行 `/cancel-ralph`。

状态持久化在 `.sisyphus/ralph-loop.local.md`，重启后可恢复。

**与 `/ulw-loop` 的区别**：`/ralph-loop` 由 agent 自判断完成，无 Oracle 验证，有迭代上限；`/ulw-loop` 需 Oracle 验证通过才结束，无迭代上限。

### 3. Hashline Edit（防错位编辑）
