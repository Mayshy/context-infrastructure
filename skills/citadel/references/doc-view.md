...
- 标准图片语法：`![描述](url)` 或 `![描述](url){width=xxx height=xxx}`
  ...
  - 若需要理解图片内容（如截图、图表），可通过 URL 拉取图片进行识别（如有视觉能力）
...
- `:::drawio{...}:::` Drawio 流程图：只有 `src` 链接，无法直接读取内容，告知用户"文档在此处包含一张 Drawio 流程图"
- `:::minder{...}:::` 思维导图：只有 `src` 链接，无法直接读取内容，告知用户"文档在此处包含一张思维导图"
- `:::gantt{...}:::` 甘特图：只有 ID 引用，无法读取详情，告知用户"文档在此处包含一张甘特图"
- `:::data2chart{...}:::` 数据图表：只有 ID 引用，无法读取数据，告知用户"文档在此处包含一张数据图表"
...
- `:::table{...}` 扩展表格（含合并单元格）：内部是 JSON 格式，每个单元格的 `content` 字段是 CitadelMD 文本，需要递归理解
...
- `:::appendix:::` + `:::footnote_list` 附录/脚注：如有脚注内容可说明
