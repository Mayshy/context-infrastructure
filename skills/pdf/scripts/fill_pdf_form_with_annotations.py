...
def _get_cjk_font_name():
    """Return a CJK-capable font name for the current platform (for FreeText annotations)."""
    system = platform.system()
    if system == 'Darwin':  # macOS
        return 'PingFang SC'  # 苹方，macOS 10.11+ 自带
    elif system == 'Windows':
        return 'Microsoft YaHei'  # 微软雅黑
    else:
        return 'Noto Sans CJK SC'  # Linux，noto-fonts-cjk 包
...
def fill_pdf_form(input_pdf_path, fields_json_path, output_pdf_path):
    ...
    for field in fields_data["form_fields"]:
        ...
        # Arial 不支持中文，含 CJK 字符时自动使用系统 CJK 字体
        font_name = entry_text.get("font")
        if font_name is None:
            font_name = _get_cjk_font_name() if _has_cjk(text) else "Arial"
